    ...
    return createPortal(
        <>
            <div className={`${mode === 'cookies' ? styles.modalCookiesOverlay : styles.modalOverlay}`} onClick={(e)=> {e.currentTarget === e.target && closeModal()}}>
                <div className={`${mode === 'cookies' ? styles.modalCookiesContent : styles.modalContent} bg-dots text-white`}>
                    <div className=" ">
                        <span className='flex justify-between text-orange font-medium text-lg p-2'>
                            <h3 className={styles.modalTitle}>{title}</h3>
                            <button onClick={closeModal} className="hover:text-white text-medium cursor-pointer">&#10005;</button>
                        </span>
                        <hr />
                    </div>
                    <span>{children}</span>
